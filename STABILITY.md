...
Snapshot as of **v0.23.0** (`const version` in `cmd/vellum`). Annotations:
...
**`viewer`** — localhost view server (HTML) + cached PDF open; macOS default Markdown handler (added in v0.6.0; view server in v0.14.0). Cache filename is `sha256(absPath)[:8]+ext` (not mtime). A `.stamp` sidecar records source mtime+size so HTML/PDF re-render in place when Markdown changes (🎯T28). HTML responses inject view chrome at serve time (TOC, figure lightbox, PDF/clipboard/Finder actions under `/_vellum/`); the convert-cache HTML file stays chrome-free (🎯T32). An open view tab watches the source over `/_vellum/watch` (WebSocket, 5-minute heartbeat) and reloads on stamp change after the file has been quiet for a moment (🎯T43). A rename redirects that tab; a deletion leaves the rendered page up and reports it; a missing file is a waiting page until the file appears. GFM task-list checkboxes can toggle the source via `POST /_vellum/task-toggle` after consent (🎯T34); the response is a source stamp that chrome adopts without `location.reload()` (🎯T44). Watch-triggered reloads restore heading-anchor or Y scroll and the focused task checkbox (or element id) from sessionStorage.
