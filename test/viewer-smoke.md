...
2. Toggle a checkbox below by clicking the box or tabbing to it and pressing Space (not the text). First click asks for consent (this session or always). The source on disk should flip `[ ]` ↔ `[x]` without reloading; keyboard focus must stay on the box. Clicking the label text must not toggle.
